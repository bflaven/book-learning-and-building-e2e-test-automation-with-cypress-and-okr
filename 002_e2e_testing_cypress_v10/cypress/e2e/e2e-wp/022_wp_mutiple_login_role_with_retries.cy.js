...
022_wp_mutiple_login_role_with_retries.cy.js
...
npx cypress run --config video=false,screenshotOnRunFailure=false --spec "cypress/integration/e2e-wp/020_wp_mutiple_login_role.cy.js"
...
npx cypress run --env LANG="en_EN",port=4222,name="Bruno",project="MySuite_2e2"  --config-file "WP_EN.conf.env.json" --config video=false,screenshotOnRunFailure=false --spec "cypress/integration/e2e-wp/020_wp_mutiple_login_role.cy.js"
...
// Note: Do not set the customization at the same and change a username or a password if you want to see a failure in a test

describe('020 :: WP :: HANDLING LOOP ::  ensure the minimum WP backend functioning @content @javascript :: Testing ' + globalValues.lang + ' (WpDebug) :: DEBUG @javascript @e2e @wp @required', 
/*
...
*/
    ...
    it('020_4 :: WP :: MUTIPLE LOGIN ROLE :: Author :: Testing ' + globalValues.lang + ' :: MUTIPLE LOGIN ROLE (WpBackMutipleLoginRole)', {
