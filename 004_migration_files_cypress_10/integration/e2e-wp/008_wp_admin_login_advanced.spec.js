...
008_wp_admin_login_advanced.spec.js
...
- launch the script only with no video and no screenshots
npx cypress run --config video=false,screenshotOnRunFailure=false --spec "cypress/integration/e2e-wp/008_wp_admin_login_advanced.spec.js"
...
describe('008_1 :: E2E-WP :: Check local WP backend :: VARIABLES ', function () {

    it('007_1_1 :: Check the env variables before accessing to WP backend', function () {
            cy.visit('/')
...
describe('008_2 :: E2E-WP :: Check local WP backend :: LOGIN ', function () {

       it ('008_2_1 :: Login to WP with your Credentials', () => {
        cy.log('\n --- go the hp')
        cy.visit('/')
        cy.wait(timer)
        cy.log('\n --- go the backend')
